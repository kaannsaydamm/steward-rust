...
if (-not (Test-Path -LiteralPath $cli) -or -not (Test-Path -LiteralPath $daemon)) {
...
if (-not $NoService) {
    $action = New-ScheduledTaskAction -Execute (Join-Path $root "steward-daemon.exe")
    $trigger = New-ScheduledTaskTrigger -AtLogOn -User $env:USERNAME
    Register-ScheduledTask -TaskName "Steward Daemon" -Action $action -Trigger $trigger -Description "Steward local agent daemon" -Force | Out-Null
}
