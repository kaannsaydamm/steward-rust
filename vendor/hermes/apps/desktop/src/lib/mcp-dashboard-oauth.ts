...
const UPDATE_BACKEND = 'Update the Steward backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Steward Desktop to support MCP OAuth callbacks.')
  }
