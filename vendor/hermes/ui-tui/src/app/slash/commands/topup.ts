...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Steward page; this is NOT a per-terminal stop.
      sys(
        "Remote spending is off for this account — a billing admin can turn it on from the portal's Steward page."
      )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into the portal — run /portal to log in, then /topup.')
