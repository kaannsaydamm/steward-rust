 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Steward"
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Steward Desktop to open another Bot chat.'
    })
