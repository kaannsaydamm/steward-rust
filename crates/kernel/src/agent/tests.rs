...
fn services(model: FakeModel, tools: Arc<RecordingTools>) -> KernelServices {
    KernelServices {
        ...
        tools,
        success: Arc::new(PolicyEvaluator {
            check: |c| match c {
...
async fn tool_action_executes_once_and_enters_history_canonically() {
    ...
    let outcome = engine
        .run(
            &services(spy_model, tools.clone()),
            history(),
            vec![],
            &Budget::default(),
            &SuccessPolicy::AgentDeclared,
    ...
    assert_eq!(
        tool_messages.len(),
        1,
        "exactly one canonical tool observation"
    );
    assert_eq!(
...
async fn multiple_actions_execute_all() {
    let model = FakeModel::new(vec![
        ...
        ModelResponse {
            actions: vec![AgentAction::Final { text: "ok".into() }],
            ..Default::default()
        },
    ]);
    let tools = Arc::new(RecordingTools::default());
    ...
    assert_eq!(
        *tools.executed.lock(),
        vec!["fs.read".to_owned(), "fs.search".to_owned()]
    );
}
...
async fn emergency_ceiling_is_configurable_and_distinct() {
    ...
    let tools = Arc::new(RecordingTools::default());
    let never = SuccessPolicy::All(vec![SuccessCriterion::FileExists {
        path: "never".into(),
    }]);
    let outcome = TurnEngine {
        emergency_ceiling: 2,
