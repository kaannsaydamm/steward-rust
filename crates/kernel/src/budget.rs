...
mod tests {
    ...
    fn unlimited_budget_always_ok() {
        ...
        assert_eq!(
            budget.check(1000, 1000, 1000, 1000, u64::MAX, u64::MAX, u64::MAX, u64::MAX),
            BudgetCheck::Ok
        );
    }
    ...
    fn model_call_budget_exhausts() {
        let budget = Budget { max_model_calls: Some(3), ..Default::default() };
    ...
    fn each_dimension_terminates_independently() {
        let cases = [
            (Budget { max_tool_calls: Some(1), ..Default::default() }, (0, 1, 0, 0), "max_tool_calls"),
            (Budget { max_spawned_agents: Some(1), ..Default::default() }, (0, 0, 1, 0), "max_spawned_agents"),
            (Budget { max_failures: Some(1), ..Default::default() }, (0, 0, 0, 1), "max_failures"),
            (Budget { max_input_tokens: Some(10), ..Default::default() }, (0, 0, 0, 0), "max_input_tokens"),
            (Budget { max_output_tokens: Some(10), ..Default::default() }, (0, 0, 0, 0), "max_output_tokens"),
            (Budget { max_cost_microusd: Some(10), ..Default::default() }, (0, 0, 0, 0), "max_cost_microusd"),
            (Budget { max_wall_clock_ms: Some(10), ..Default::default() }, (0, 0, 0, 0), "max_wall_clock_ms"),
        ];
        for (budget, (m, t, s, f), expected) in cases {
            let (i, o) = if expected == "max_input_tokens" { (10u64, 0u64) } else if expected == "max_output_tokens" { (0, 10) } else { (0, 0) };
            let cost = if expected == "max_cost_microusd" { 10u64 } else { 0u64 };
            let elapsed = if expected == "max_wall_clock_ms" { 10u64 } else { 0u64 };
            assert_eq!(
