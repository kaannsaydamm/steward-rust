...
export const zh: Translations = {
  sessionImport: {
    title: '从其他应用继续',
    subtitle: '将对话导入 Steward，接着上次的进度继续。',
    action: '导入会话',
    readingFrom: '读取自',
  ...
  boot: {
    ready: 'Steward 桌面版已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败：${message}`,
    steps: {
      connectingGateway: '正在连接桌面网关',
      loadingSettings: '正在加载 Steward 设置',
      loadingSessions: '正在加载最近会话',
      retryingRemoteBackend: '正在重新连接远程 Steward 后端…',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Steward 桌面版…'
    },
    errors: {
      backgroundExited: 'Steward 后台进程已退出。',
      backgroundExitedDuringStartup: 'Steward 后台进程在启动期间退出。',
    ...
    failure: {
      ...
      cloudDownTitle: 'Steward Cloud 代理已宕机',
      cloudDownDescription:
        '此网关连接的 Steward 托管云代理正在返回服务器错误。无法在此处重启——请检查其状态、切换到本地网关或获取支持。',
      cloudDownHint: '使用下方按钮打开 Steward Portal（查看实例状态与操作）或加入 Discord 获取支持。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Steward 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Steward Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'Steward 正在等待你的回应。',
      turnDoneTitle: 'Steward 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  sendDiagnostics: {
    title: '向 Steward 发送诊断信息',
    privacyNotice:
      '这会将调试包上传到 Steward 内部存储（并非公开粘贴板）。内容包括系统信息（操作系统、版本、服务商、已配置的 API 密钥种类 — 绝不包含密钥本身）以及完整的 agent、gateway 和桌面端日志（每个最多 512 KB，很可能包含对话内容、工具输出与文件路径）。上传前会先脱敏。仅 Steward 员工与获准的 Discord 版主可查看，14 天后自动删除。',
    upload: '上传',
    uploading: '上传中…',
    cancel: '取消',
    close: '关闭',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Steward 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Steward 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Steward 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 Steward 后端以便在桌面端管理此插件。',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 Steward 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'Steward 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Steward 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: 'Steward 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: 'Steward',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Steward 跟随系统设置。',
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，Steward 也能回应你的消息。',
      tipsTitle: '应用内提示',
      tipsDesc: '指向应用某处的小气泡：空闲时偶尔出现，需要时 Steward 也会给你一条。关掉一条就不再出现。',
      tipsReset: (count: number) => `恢复 ${count} 条已关闭的提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Steward 带你熟悉应用：调暗界面并逐步高亮每个位置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Steward 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Steward，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Steward 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Steward 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Steward 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Steward 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Steward 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Steward 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      bundleOutOfSyncAction: '获取安装程序',
      bundleSwapPending: '重启以完成更新',
      bundleSwapPendingDesc: '更新后的应用已安装完成，只需重启 Steward 即可加载新版本。聊天记录和设置不会受到影响。',
      bundleSwapPendingAction: '重启 Steward',
      updates: '更新',
      checkNow: '立即检查',
      checking: '检查中…',
      seeWhatsNew: '查看新增内容',
      ...
      automaticUpdatesDesc: 'Steward 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 Steward 即可发送提示。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Steward Cloud 连接可访问的每个 Steward 网关。',
      stagedNote:
      ...
      updateSkippedCloud: '由 Steward Cloud 托管',
      kindLocal: '本地',
      kindRemote: '远程网关',
      kindCloud: 'Steward Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 Steward 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 Steward 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Steward Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Steward 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 Steward 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
      ...
      cloudAddHint: '提示：在上方登录 Steward Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Steward Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Steward 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      envOverrideTitle: '环境变量正在控制此桌面会话。',
      envOverrideDesc: '取消设置 HERMES_DESKTOP_REMOTE_URL 和 HERMES_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Steward 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Steward 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'Steward Cloud',
      cloudDesc: '只需登录 Steward Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。',
      cloudSignInTitle: 'Steward Cloud',
      cloudSignIn: '登录 Steward Cloud',
      cloudSignedIn: '已登录 Steward Cloud',
      cloudNeedsSignIn: '登录 Steward Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 Steward Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'Steward Cloud 登录失败',
      cloudSignedOutTitle: '已退出 Steward Cloud',
      cloudSignedOutMessage: '已清除 Steward Cloud 会话。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Steward Desktop 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Steward ${version}` : ''}`,
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'Steward 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshHermesPathTitle: 'Steward 路径（可选）',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 Steward`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——Steward 以非交互方式运行 ssh。',
      sshErrHostKey: '自上次连接以来主机密钥已更改。请确认这是预期的，然后运行 ssh-keygen -R <host> 并重新连接。',
      sshErrNotInstalled:
        '远程主机上未安装 Steward。请在远程安装（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或设置 Steward 路径。',
      sshErrPlatform: '不支持的远程平台。Steward Desktop 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 Steward。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 Steward。请检查下方的完整配置——它来自该链接，而非 Steward。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `构建 ${tag}，${backend} 后端。Steward 会为您启动并管理服务器。`,
      ...
      upToDateDetail: (tag, backend) => `正在运行 llama.cpp ${tag}（${backend}）——Steward 提供的最新构建。`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Steward 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Steward 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
    ...
    toolsets: {
      ...
      useBackend: '使用此后端',
      nousIncluded: '包含在 Steward 订阅中；登录 Steward Portal 即可激活。',
      nousAuthNeededTitle: '登录 Steward Portal',
      nousAuthNeededMessage: provider => `已保存 ${provider}，但在登录 Steward Portal 之前不会激活。`,
      nousAuthSignIn: '登录',
      nousAuthDoneTitle: '已连接 Steward Portal',
      nousAuthDoneMessage: '订阅后端现已激活。',
      nousAuthFailed: 'Steward Portal 登录未完成',
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '让网站保持登录状态',
          body: '让 Steward 使用默认浏览器配置文件的快照进行浏览，网站打开时即已登录。',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Steward 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Steward 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Steward 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Steward 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Steward 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Steward ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    openBrowser: '打开浏览器',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Steward 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Steward 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Steward 指向该 URL。',
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Steward 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Steward 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此配置档案中的会话将在你指定的远程 Steward 上运行，而不是这台电脑。',
      urlLabel: '远程地址',
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Steward 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Steward 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Steward 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Steward 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 Steward 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Steward…',
    placeholderReconnecting: '正在重新连接 Steward…',
    ...
    attachUrlDesc: 'Steward 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Steward 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Steward 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Steward 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Steward，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Steward 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Steward 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Steward 更新器会在自己的窗口中接管，并在完成后自动重新打开 Steward。更新期间请不要自行重新打开 Steward。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Steward 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Steward 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 Steward？',
    blockerBody: 'Steward 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 Steward',
    foreignBlockerBody: 'Steward 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'Steward 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 Steward Desktop',
    setupChoiceDesc: '将此应用连接到你已运行的 Steward 网关，或在这台电脑上本地安装 Steward。',
    connectExistingTitle: '连接到现有 Steward',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 Steward',
    installLocalDesc: '下载 Steward，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 Steward Desktop 后重试。',
    remoteSetupTitle: '连接到现有 Steward',
    remoteSetupDesc: '输入网关 URL。Steward Desktop 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 Steward 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 Steward 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Steward',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Steward CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Steward 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Steward 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 Steward',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'Steward 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Steward…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Steward 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Steward 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Steward 会自动连接',
    ...
    authorizeThere: '请在那里授权 Steward。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Steward，连接会自动完成，无需复制或粘贴。`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付费 Steward 订阅。',
    free: '免费',
    freeTier: '免费层',
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Steward 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Steward 正在重启...',
      askRestart: '让 Steward 重启服务器',
      lookingRestart: taskId => `Steward 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Steward 正在后台工作。可在预览控制台查看进度。',
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Steward 打开一个页面。',
      finishedRestarting: message => `Steward 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Steward 无法重启服务器。',
      stillWorking: 'Steward 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'Steward 正在加载回复',
    ...
    approval: {
      gatewayDisconnected: 'Steward 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Steward 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Steward 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Steward 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Steward 桌面版以保存图片',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就够',
        text: '技能是一组说明文件，Steward 会在需要时自行加载。'
      },
      messaging: {
        title: '离开电脑也能用',
      ...
      profiles: {
        title: '配置文件彼此独立',
        text: '每个都是独立的 Steward — 自己的密钥、记忆和会话。'
      },
      'composer-mentions': {
        title: '附件与命令',
