...
const BUILTIN_NAV_REST: NavItem[] = [
  ...
  { path: "/kg", label: "Knowledge", icon: Network },
  {
    path: "/models",
    labelKey: "models",
    label: "Models",
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'steward update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "steward update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'steward update' (${cmd}) and restart the gateway when it finishes.`
    );
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Steward gateway process. Connected channels and active sessions will reconnect afterward."
      }
