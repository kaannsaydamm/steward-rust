 ...
 *  clients (Steward Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Steward"
 *  (community report, Aug 21 2026: renamed default still read "Steward is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Steward (matching displayName and the
...
export function groupSpeakerLabel(name?: null | string) {
  ...
  return trimmed.toLowerCase() === 'default' ? 'Steward' : trimmed
}
