...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Steward и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Steward.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Steward. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Steward',
    continue: 'Продолжить в Steward',
  ...
  boot: {
    ready: 'Steward Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Steward',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Steward…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingHermesDesktop: 'Запуск Steward Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Steward завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Steward завершился при запуске.',
    ...
    failure: {
      title: 'Steward не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Steward старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateHermes: 'Обновить Steward',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Steward Desktop.',
    ...
    native: {
      ...
      inputBody: 'Steward ожидает ваш ответ.',
      turnDoneTitle: 'Steward завершил',
      turnDoneBody: '',
  ...
  billingBlock: {
    titleNous: 'Кредиты Steward закончились',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Steward по умолчанию?',
    ...
    plugins: {
      ...
      agent: {
        ...
        blurb:
          'Плагины, установленные в бэкенд Steward — инструменты, навыки, MCP-серверы, хуки и slash-команды. Переносимые — пакеты Agent Plugins (навыки + MCP-бандлы, работающие и в других агентах). Переключатели действуют для новых сеансов.',
        ...
        updateBackendToManage: 'Обновите бэкенд Steward, чтобы управлять этим плагином из приложения.',
        sources: {
          bundled: 'встроенный',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Steward в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Steward задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Steward был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Steward был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Steward следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Steward сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Steward — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Steward и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Steward останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Steward может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Steward обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Steward обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Steward обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Steward автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации Steward…',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Steward.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Steward, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Steward Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Steward Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Steward, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Steward, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Steward Cloud.',
      kindSshDesc: 'Установка Steward, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Steward (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Steward Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Steward в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      ...
      localDesc: 'Запускает приватный бэкенд Steward на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Steward.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в Steward Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Steward Cloud',
      cloudSignIn: 'Войти в Steward Cloud',
      cloudSignedIn: 'Вы вошли в Steward Cloud',
      cloudNeedsSignIn: 'Войдите в Steward Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Steward Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Steward Cloud',
      cloudSignedOutTitle: 'Выход из Steward Cloud',
      cloudSignedOutMessage: 'Сессия Steward Cloud сброшена.',
      ...
      restartingMessage: 'Steward Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Steward ${version}` : ''}`,
      ...
      sshDesc:
        'Steward запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshHermesPathTitle: 'Путь к Steward (необязательно)',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Steward найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Steward запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Steward не установлен на удалённой машине. Установите его там (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) или задайте путь к Steward.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Steward Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Steward на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Steward. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Steward.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Steward проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Steward на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
    ...
    toolsets: {
      ...
      nousIncluded: 'Входит в подписку Steward — войдите в Steward Portal, чтобы активировать.',
      nousAuthNeededTitle: 'Войдите в Steward Portal',
      nousAuthNeededMessage: provider => `${provider} сохранён, но не активируется, пока вы не войдёте в Steward Portal.`,
      nousAuthSignIn: 'Войти',
      nousAuthDoneTitle: 'Steward Portal подключён',
      nousAuthDoneMessage: 'Ваши бэкенды по подписке теперь активны.',
      nousAuthFailed: 'Вход в Steward Portal не завершён',
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Steward создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Steward, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Steward, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Steward уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Steward desktop' },
    ...
    hermesActiveSessions: (version, count) => `Steward ${version} · Активные сеансы: ${count}`,
  ...
  profiles: {
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Steward: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'Steward не сохранил это изменение модели.'
    },
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Steward выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Steward, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Steward. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Steward, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Steward...',
    placeholderReconnecting: 'Переподключение к Steward…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте Steward задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'Steward загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Steward открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Steward…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Steward…',
    ...
    unsupportedMessage: 'Эта версия Steward не может обновлять себя из приложения.',
    ...
    availableBody: 'Новая версия Steward готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Steward готова к установке.',
    ...
    manualBody:
      'Steward установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Steward подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Steward (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Steward возьмёт процесс на себя в своём окне и автоматически перезапустит Steward, когда закончит. Пожалуйста, не открывайте Steward вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Steward переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Steward откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Steward?',
    blockerBody:
      'Steward нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Steward',
    foreignBlockerBody:
      'Steward не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Steward может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  install: {
    ...
    oneTimeTitle: 'Steward требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Steward Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Steward или установите Steward локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Steward',
    ...
    installLocalTitle: 'Установить Steward локально',
    installLocalDesc: 'Скачайте Steward, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Steward Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Steward',
    remoteSetupDesc: 'Введите URL вашего шлюза. Steward Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Steward, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Steward.',
    ...
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Steward CLI или desktop. Остановите все работающие инстанции Steward и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Steward скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    ...
    preparingInstall: 'Steward завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Steward…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Steward',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Steward локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Steward подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Steward там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Steward там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Модели Pro требуют платной подписки Steward.',
    free: 'Free',
    freeTier: 'Бесплатный тариф',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Steward покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Steward перезапускается...',
      askRestart: 'Попросить Steward перезапустить сервер',
      lookingRestart: taskId => `Steward ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Steward работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Steward открыть страницу.',
      finishedRestarting: message => `Steward завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Steward не смог перезапустить сервер.',
      stillWorking:
        'Steward всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Steward загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Steward не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.hermes/config.yaml). Steward больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
    ...
    mcpSetup: {
      ...
      catalogSource: 'Из каталога, одобренного Steward',
  ...
  prompts: {
    ...
    sudoDesc:
      'Steward нужен ваш пароль sudo, чтобы выполнить команду с повышенными правами. Он отправляется только вашему локальному агенту.',
    sudoPlaceholder: 'пароль sudo',
    secretTitle: 'Требуется секрет',
    secretDesc: 'Steward нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Steward Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Steward Desktop, чтобы сохранять изображения',
