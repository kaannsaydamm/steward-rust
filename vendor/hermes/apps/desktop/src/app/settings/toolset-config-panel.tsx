 ...
 * server-computed `status` (keys ∧ Steward entitlement ∧ post-setup install
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the Steward Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. Steward Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_nous_auth) {
        // Managed Steward row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing Steward Portal device-code flow.
        notify({
          kind: 'warning',
  ...
  // Drive the existing Steward Portal OAuth device-code flow (the same session
