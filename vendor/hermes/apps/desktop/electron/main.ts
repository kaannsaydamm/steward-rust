...
//
// Steward parity default: ~/.steward (the daemon's state root — steward.db,
// wire-port, providers.json, agents/*.yaml). Sessions and profiles the
// desktop's `hermes serve` backend writes must land in the SAME store the
// `steward` CLI and the web dashboard read; the HERMES_HOME env override
// (and the Windows user-scope registry value) still win for anyone who
// re-homes the install.
//
// HERMES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// HERMES_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.steward.
function resolveHermesHome() {
  ...
  if (IS_WINDOWS) {
    ...
    // backend silently falls back to a platform default and reports "No
  ...
  return path.join(app.getPath('home'), '.steward')
}
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Steward'
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Steward backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
let bootProgressState = {
  ...
  message: 'Waiting to start Steward backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Steward will start automatically when it completes…',
        12
      )
    },
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Steward to load the updated app…', 14)
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Steward at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveHermesBackend(backendArgs) {
  ...
  return {
    ...
    label: 'Steward backend not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Steward recovery was handed off to Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Steward install was cancelled.') as any
  ...
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Steward runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Steward Cloud agent…' : 'Sign in to Steward gateway',
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Steward Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Steward Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Hermes Cloud session has expired. Sign in to Steward Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Steward backend for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Steward Desktop is already running in another window.')
  }
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Steward backend start was superseded by a newer connection attempt.')
      }
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        ...
        rejectBackendStart?.(new Error('Steward backend start was superseded by a newer connection attempt.'))
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Steward backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Steward is not installed yet' }
    }
