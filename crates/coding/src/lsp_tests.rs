...
async fn handshake_is_performed_once_and_cached() {
    ...
    let session = manager
        .start_server("/repo", "rust", server.clone())
        .await
        .unwrap();
    manager
        .start_server("/repo", "rust", server.clone())
...
async fn hover_returns_normalized_content() {
    let manager = LspManager::default();
    let session = manager
        .start_server("/repo", "rust", Arc::new(MockServer::default()))
        .await
        .unwrap();
    let hover = manager
        .hover(&session, "/repo/src/lib.rs", 5, 1)
        .await
        .unwrap();
...
async fn definition_returns_file_line_column_one_indexed() {
    ...
    let locations = manager
        .definition(&session, "/repo/src/lib.rs", 5, 1)
        .await
        .unwrap();
...
async fn references_return_exact_ranges() {
    ...
    let references = manager
        .references(&session, "/repo/src/lib.rs", 1, 1, false)
        .await
        .unwrap();
...
async fn diagnostics_map_severity_and_lines() {
    ...
    let manager = LspManager::default();
    let session = manager
        .start_server("/repo", "rust", Arc::new(server))
        .await
        .unwrap();
    let diagnostics = manager
        .diagnostics(&session, "/repo/src/broken.rs")
        .await
        .unwrap();
...
async fn rename_returns_workspace_edit_for_atomic_apply() {
    ...
    let session = manager
        ...
        .unwrap();
    let edits = manager
        .rename(&session, "/repo/src/a.rs", 1, 5, "renamed_kernel")
        .await
        .unwrap();
