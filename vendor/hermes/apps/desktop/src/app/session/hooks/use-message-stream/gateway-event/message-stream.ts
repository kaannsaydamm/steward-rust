 ...
 * billing-specific toast — never the generic "Steward error" — with a smart CTA
 * (Steward → in-app Settings → Billing, other providers → their billing page).
 */
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Steward reported an error',
